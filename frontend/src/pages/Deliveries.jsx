                ...
                {filtered.map(order => (
                  <motion.div key={order.id} {...fadeUp} layout
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      {/* Fardar copy helper */}
                      {order.courier_name?.toLowerCase().includes('fardar') && (
                        <button onClick={() => {
                          const name = order.receiver_name || order.customer_name || '';
                          const phone = order.receiver_phone || order.customer_phone || '';
                          const addr = order.receiver_address || order.delivery_address || '';
                          const cod = Number(order.grand_total) || 0;
                          const items = order.items?.length ? order.items.map(i => `${i.product_name}${i.variant_label ? ' - ' + i.variant_label : ''} x${i.quantity}`).join(', ') : `Order ${order.order_number}`;
                          const text = `Recipient Name: ${name}\nContact: ${phone}\nAddress: ${addr}\nCOD Amount: ${cod}\nOrder ID: ${order.order_number}\nDescription: ${items}`;
                          navigator.clipboard.writeText(text);
                          toast.success('Courier details copied to clipboard');
                        }}
                          className="flex items-center gap-1 rounded-lg border border-amber-200 bg-amber-50 px-2.5 py-1.5 text-xs font-medium text-amber-700 transition hover:bg-amber-100 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-400"
                          title="Copy these details, then paste into Fardar's order form">
